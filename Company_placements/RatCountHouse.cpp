...
rat consumes and each ith element of array ‘arr’ represents the amount of food present in ‘i+1’ house number,
where 0 <= i
...
int foodSufficiency(int arr[], int n, int r, int unit)
{
    int sum = 0;
    if (n == 0)
        return -1;
...
int main()
{
    int r, unit, arr[1000], n;
    cin >> r >> unit >> n;
    for (int i = 0; i < n; i++)
        cin >> arr[i];
    cout << foodSufficiency(arr, n, r, unit) << endl;
    return 0;
}